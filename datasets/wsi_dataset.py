...
def get_contour_check_fn(
    contour_fn="four_pt_hard", cont=None, ref_patch_size=None, center_shift=None
):
    if contour_fn == "four_pt_hard":
        cont_check_fn = isInContourV3_Hard(
            contour=cont, patch_size=ref_patch_size, center_shift=center_shift
        )
    elif contour_fn == "four_pt_easy":
        cont_check_fn = isInContourV3_Easy(
            contour=cont, patch_size=ref_patch_size, center_shift=0.5
        )
...
class Wsi_Region(Dataset):
    ...
    def __init__(
        ...
        self.ref_size = tuple(
            (np.array(patch_size) * np.array(self.ref_downsample)).astype(int)
        )
        ...
        for cont_idx, contour in enumerate(wsi_object.contours_tissue):
            print(
                "processing {}/{} contours".format(
                    cont_idx, len(wsi_object.contours_tissue)
                )
            )
            cont_check_fn = get_contour_check_fn(
                contour_fn, contour, self.ref_size[0], center_shift
            )
            coord_results, _ = wsi_object.process_contour(
                contour,
    ...
    def __getitem__(self, idx):
        ...
        patch = self.wsi.read_region(tuple(coord), self.level, self.patch_size).convert(
            "RGB"
        )
        if self.custom_downsample > 1:
