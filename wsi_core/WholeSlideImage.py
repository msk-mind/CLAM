...
class WholeSlideImage(object):
    ...
    def segmentTissue(
        ...
        def _filter_contours(contours, hierarchy, filter_config):
            """
            ...
            for cont_idx in hierarchy_1:
                ...
                if a == 0:
                    continue
                if tuple((filter_config["a_t"],)) < tuple((a,)):
            ...
            for hole_ids in all_holes:
                ...
                unfilered_holes = unfilered_holes[: filter_config["max_n_holes"]]
                filtered_holes = []

                # filter these holes
                for hole in unfilered_holes:
                    if cv2.contourArea(hole) > filter_config["a_h"]:
        ...
        filter_config["a_t"] = filter_config["a_t"] * scaled_ref_patch_area
        filter_config["a_h"] = filter_config["a_h"] * scaled_ref_patch_area
        ...
        if filter_config:
            foreground_contours, hole_contours = _filter_contours(
                contours, hierarchy, filter_config
    ...
    def _getPatchGenerator(
        ...
        log.info(f"Bounding Box: {start_x},{start_y} {w}x{h}")
    ...
    def process_contours(
        ...
        **kwargs,
        ...
        log.info(
            f"Creating patches for: {self.name}...",
        )
        elapsed = time.time()
    ...
    def process_contour(
        ...
        patch_downsample = (
            int(self.level_downsamples[patch_level][0] * custom_downsample),
        ...
        if bot_right is not None or top_left is not None:
            ...
            if w <= 0 or h <= 0:
                ...
                log.info(f"Adjusted Bounding Box: {start_x},{start_y} {w}x{h}")
        ...
        if len(results) > 1:
            ...
            attr = {
                "patch_size": ref_patch_size[0],  # To be considered...
                "patch_level": patch_level,
    ...
    def visHeatmap(
        ...
        log.info(f"top_left: {top_left}, bot_right: {bot_right}")
        log.info("w: {}, h: {}".format(w, h))
        log.info(f"scaled patch size: {patch_size}")
